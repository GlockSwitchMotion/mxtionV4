...
run(function()
    ...
    PreventSlow = vape.Categories.Blatant:CreateModule({
        Name = 'PreventSlow',
        Function = function(callback)
            ...
            if callback then
                ...
                modifier.addModifier = function(self, tab)
                    if tab.moveSpeedMultiplier then
                        tab.moveSpeedMultiplier = math.max(tab.moveSpeedMultiplier, SpeedMultiplier.Value)
                    end
                    return old(self, tab)
                end

                for i in modifier.modifiers do
                    if (i.moveSpeedMultiplier or 1) < SpeedMultiplier.Value then
    ...
    SpeedMultiplier = PreventSlow:CreateSlider({
        Name = 'Speed Multiplier',
        Min = 0.1,
        Max = 1,
        Default = 1,
        Decimal = 100,
        Tooltip = 'Minimum movement speed multiplier allowed when slowed'
    })
end)
...
run(function()
	...
	Speed = vape.Categories.Blatant:CreateModule({
		...
		Tooltip = 'Increases your speed  with zephyr.'
	})
...
run(function()
	...
	vape.Components.HotbarList = function(optionsettings, children, api)
		...
		textbuttonicon.Image = getcustomasset('mxtionv4/assets/new/add.png')
		...
		function optionapi:AddHotbar(data)
			...
			close.BackgroundColor3 = Color3.new(1, 1, 1)
			close.BackgroundTransparency = 1
			close.Image = getcustomasset('mxtionv4/assets/new/closemini.png')
			close.ImageColor3 = color.Light(uipallet.Text, 0.2)
			close.ImageTransparency = 0.5
			close.AutoButtonColor = false
			close.Parent = hotbar
...
run(function()
	...
	MotionBlur = vape.Categories.World:CreateModule({
		Name = 'MotionBlur',
		Function = function(callback)
			if callback then
            ...
            if call then
                repeat
                    if entitylib.isAlive and store.equippedKit == 'dragon_slayer' and bedwars.AbilityController:canUseAbility('dragon_slayer_punch', {disableBlockedAbilityAlert = true}) then
                        local localPosition = entitylib.character.RootPart.Position
                        local requiredStacks = stackValues[StackMode.Value] or 1
                        
                        for target, v in bedwars.DragonSlayerController.dragonEmblems do
                            if v.stackCount >= requiredStacks and target.PrimaryPart and (target.PrimaryPart.Position - localPosition).Magnitude <= Range.Value then
                                punch()
                                break
                            end
                        end
				...
				local mounted = roact.mount(create('BillboardGui', {
					...
					create('Frame', {
						...
						create('ImageLabel', {
							...
							BackgroundTransparency = 1,
							Image = getvapeasset('mxtionv4/assets/new/blur.png'),
...
run(function()
	...
	NameTagSpoofer = vape.Categories.Render:CreateModule({
		Name = 'NameTagHider',
		Tooltip = 'hide your nametag & customize',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local state = {
		mode = 'searching', -- searching, collecting, rushing, breaking, combat
		targetEnemy = nil,
		targetIron = nil,
		targetBed = nil,
		lastCollectTime = 0,
	...
	local function findClosestEnemy()
		...
		for _, ent in ipairs(entitylib.List) do
			...
			if ent.Humanoid.Health <= 0 then continue end
			if ent.Player and entitylib.isFriend and entitylib.isFriend(ent.Player) then continue end
			
			local myTeam = lplr:GetAttribute('Team')
			local entTeam = ent.Player:GetAttribute('Team')
			if myTeam == entTeam then continue end
	...
	local function findClosestIron()
		if not entitylib.isAlive then return nil end
		local root = entitylib.character.RootPart
		local closest, closestDist = nil, 50
		
		local irons = collection('iron_ore_mesh_block')
		for _, ore in ipairs(irons or {}) do
			if not ore.Parent then continue end
			local dist = (ore.Position - root.Position).Magnitude
			if dist < closestDist then
				closest, closestDist = ore, dist
			end
		end
		return closest
	end
	...
	local function findEnemyBed()
		...
		local beds = collection('bed')
		for _, bed in ipairs(beds or {}) do
			if not bed.Parent then continue end
			
			-- DONT BREAK OWN BED
			local bedPlacedBy = bed:GetAttribute('PlacedByUserId')
			if bedPlacedBy == myUserId then continue end
			
			local bedPlayer = game:GetService('Players'):GetPlayerByUserId(bedPlacedBy)
			if bedPlayer then
				local myTeam = lplr:GetAttribute('Team')
				local bedTeam = bedPlayer:GetAttribute('Team')
				
				-- Only target enemy beds
				if bedTeam ~= myTeam then
					local dist = (bed.Position - root.Position).Magnitude
					if dist < closestDist then
						closest, closestDist = bed, dist
					end
				end
			end
		end
	...
	local function getBlockCount()
		local count = 0
		local wool, amount = getWool()
		if wool then count = amount or 0 end
	...
	local function smartScaffold(targetPos)
		if not entitylib.isAlive then return false end
		local root = entitylib.character.RootPart
		local blockCount = getBlockCount()
		
		if blockCount < 5 then return false end
		
		local direction = (targetPos - root.Position).Unit
		local distance = (targetPos - root.Position).Magnitude
		
		if distance > 10 then
			local checkPos = root.Position + direction * 3
			if checkPos.Y < root.Position.Y then return false end
			
			local ray = workspace:Raycast(root.Position, Vector3.new(0, -10, 0), rayCheck)
			if not ray or ray.Distance > 10 then
				local wool, amount = getWool()
				if wool and amount and amount > 0 then
					bedwars.placeBlock(roundPos(root.Position + Vector3.new(0, -3, 0)), wool, false)
					return true
				end
			end
		end
	...
	local function getBlocksInInventory()
		local blockType = getWool()
		if blockType then return blockType end
	...
	local function smartCombat(enemy, dt)
		if not enemy or not enemy.RootPart then return end
		local root = entitylib.character.RootPart
		local targetRoot = enemy.RootPart
		local myHumanoid = entitylib.character.Humanoid
		local enemyHumanoid = enemy.Humanoid
		
		local distance = (root.Position - targetRoot.Position).Magnitude
		
		-- Check if enemy is attacking us - back away
		local enemyVelocity = (targetRoot.AssemblyLinearVelocity or targetRoot.Velocity or Vector3.zero).Magnitude
		if enemyVelocity > 20 and distance < 15 then
			-- Enemy rushing - back up and strafe
			state.strafeDir = -state.strafeDir
		end
		
		-- Smart strafe at 17 studs distance
		state.strafeTime = state.strafeTime + (dt * 3 * state.strafeDir)
		local angle = state.strafeTime
		
		local strafeRadius = 17
		local offsetX = math.cos(angle) * strafeRadius
		local offsetZ = math.sin(angle) * strafeRadius
		
		-- Higher if enemy is below
		local heightOffset = targetRoot.Position.Y > root.Position.Y and 3 or 0
		local combatPos = targetRoot.Position + Vector3.new(offsetX, heightOffset, offsetZ)
		
		-- Raycast to avoid walls
		rayCheck.FilterDescendantsInstances = {lplr.Character, gameCamera, AntiFallPart}
		rayCheck.CollisionGroup = root.CollisionGroup
		local ray = workspace:Raycast(targetRoot.Position, combatPos - targetRoot.Position, rayCheck)
		if ray then
			combatPos = ray.Position - (combatPos - targetRoot.Position).Unit * 3
		end
		
		-- Move to strafe position
		moveToward(combatPos)
		
		-- Smart attack logic
		if distance < 12 then
			-- Close enough to hit
			if store.hand.tool then
				prediction.trackShot()
				bedwars.Handler:Get('Attack'):Fire('CallServer', {targetRoot})
			end
			
			-- Shield if taking damage
			if myHumanoid.Health < myHumanoid.MaxHealth * 0.7 then
				if getItem('shield') then
					bedwars.selectItemByType('shield')
				end
			end
		end
		...
		if myHumanoid.Health < 10 then
			if getItem('heal') then
				bedwars.selectItemByType('heal')
				bedwars.Handler:Get('Drink'):Fire('CallServer', {})
			end
		end
	end
	
	AutoBot = vape.Categories.Minigames:CreateModule({
		Name = 'AutoBot',
		ConfigName = 'AutoBot',
		Function = function(callback)
			frictionTable.AutoBot = callback or nil
			updateVelocity()
			
			if callback then
				AutoBot:Clean(runService.PreSimulation:Connect(function(dt)
					if not entitylib.isAlive then return end
					...
					-- Smart state machine
					if enemy and (enemy.RootPart.Position - root.Position).Magnitude < 25 and CombatStrat.Enabled then
						state.mode = 'combat'
						state.targetEnemy = enemy
					elseif bed and (bed.Position - root.Position).Magnitude < 60 and blockCount >= 5 and BedBreaker.Enabled then
						state.mode = 'breaking'
						state.targetBed = bed
					elseif blockCount < IronTarget.Value then
						state.mode = 'collecting'
						state.targetIron = findClosestIron()
					elseif enemy and blockCount >= 10 then
						state.mode = 'rushing'
						state.targetEnemy = enemy
					else
					...
					-- Execute state
					if state.mode == 'breaking' and state.targetBed and state.targetBed.Parent then
						moveToward(state.targetBed.Position + Vector3.new(0, 3, 0))
						
						-- Use nuker to break bed
						if (root.Position - state.targetBed.Position).Magnitude < 20 then
							if vape.Modules.Nuker then
								vape.Modules.Nuker.Enabled = true
							end
						end
						
					elseif state.mode == 'combat' and state.targetEnemy and state.targetEnemy.RootPart then
						smartCombat(state.targetEnemy, dt)
						
					elseif state.mode == 'collecting' and state.targetIron and state.targetIron.Parent then
						moveToward(state.targetIron.Position)
						
					elseif state.mode == 'rushing' and state.targetEnemy and state.targetEnemy.RootPart then
						local enemyPos = state.targetEnemy.RootPart.Position
						smartScaffold(enemyPos)
						moveToward(enemyPos + Vector3.new(0, 5, 0))
						
					elseif state.mode == 'searching' then
						-- Idle or move to resources
					end
					
					-- Auto equip best block
					if blockCount > 0 then
						local wool = getBlocksInInventory()
						if wool and store.hand.tool ~= wool then
							bedwars.selectItemByType(wool)
						end
					end
					
					-- Fly if stuck high (no blocks left)
					if blockCount < 3 then
						local ray = workspace:Raycast(root.Position, Vector3.new(0, -20, 0), rayCheck)
						if not ray then
							if vape.Modules.InfiniteFly then
								vape.Modules.InfiniteFly.Enabled = true
							end
						end
					end
				end))
		...
		Tooltip = 'Smart bedwars bot - ignores own bed'
	})
	
	Mode = AutoBot:CreateDropdown({
		Name = 'Priority',
		List = {'Bed Rush', 'Survival', 'Iron First'},
		Default = 'Bed Rush'
	})
	
	IronTarget = AutoBot:CreateSlider({
		Name = 'Iron Target',
		Min = 5,
		Max = 32,
		Default = 16,
	...
	BedBreaker = AutoBot:CreateToggle({
		Name = 'Auto Break Bed',
		Default = true
	})
	
	CombatStrat = AutoBot:CreateToggle({
		Name = 'Combat Enabled',
		Default = true
	})
end)
...
run(function()
    ...
    StrafeSpeedSlider = PlayerAttachModule:CreateSlider({
        ...
        Max = 10,
        Default = 5,
        Tooltip = 'Speed of the auto strafe movement'
    })

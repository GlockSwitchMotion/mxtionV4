...
run(function()
	...
	local function findChickenDeploy()
		-- Search in player GUI
		local playerGui = lplr:WaitForChild("PlayerGui")
		
		-- Look for chicken_deploy in the GUI hierarchy
		local function searchGui(parent)
			for _, child in pairs(parent:GetDescendants()) do
				if child.Name == "chicken_deploy" then
					return child
				end
			end
			return nil
	...
	local function getChickenAmount()
		local chickenGui = findChickenDeploy()
		if not chickenGui then return 0 end
		
		-- Try to find amount in TextLabel or similar
		local amount = 0
		if chickenGui:IsA("TextLabel") or chickenGui:IsA("TextButton") then
			local text = chickenGui.Text
			amount = tonumber(text) or 0
		else
			-- Search for amount in children
			for _, child in pairs(chickenGui:GetDescendants()) do
				if child:IsA("TextLabel") and child.Name == "Amount" or child.Name:lower():find("amount") then
					amount = tonumber(child.Text) or 0
					break
				end
			end
		end
	...
	local function notifyChickenChange(amount)
		if vape and vape.Notifications and vape.Notifications.CreateNotification then
			vape.Notifications.CreateNotification("Taliyah Spy", "Chickens: " .. amount, 3, "assets/WarningNotification.png")
		end
	end
	...
	vape.Categories.Utility:CreateModule({
		Name = "TaliyahSpy",
		Function = function(call)
			taliyahSpy.enabled = call
			
			if call then
				taliyahSpy.lastAmount = 0
				
				-- Monitor chicken_deploy for changes
				local monitorConnection
				monitorConnection = runService.Heartbeat:Connect(function()
					if not taliyahSpy.enabled then
						monitorConnection:Disconnect()
						return
					end
					
					local currentAmount = getChickenAmount()
					if currentAmount ~= taliyahSpy.lastAmount then
						taliyahSpy.lastAmount = currentAmount
						notifyChickenChange(currentAmount)
					end
				end)
				
				taliyahSpy.connection = monitorConnection
			else
				if taliyahSpy.connection then
					taliyahSpy.connection:Disconnect()
					taliyahSpy.connection = nil
				end
			end
		end
	})

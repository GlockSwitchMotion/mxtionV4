...
run(function()
    ...
    AutoClicker = vape.Categories.Combat:CreateModule({
        ...
        Function = function(callback)
            if callback then
                -- Helper table to check valid trigger buttons (Left Click, Right Click, and Extra/Side Mouse Buttons)
                local validButtons = {
                ...
                AutoClicker:Clean(inputService.InputBegan:Connect(function(input)
                    if validButtons[input.UserInputType] then
                        AutoClick()
                    end
                end))
    
                AutoClicker:Clean(inputService.InputEnded:Connect(function(input)
                    if validButtons[input.UserInputType] and Thread then
                        task.cancel(Thread)
                        Thread = nil
                    end
                end))
...
run(function()
            ...
            if callback then
                DamageBoost:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
                    if entitylib.isAlive and tick() > (stack or 0) and damageTable.entityInstance == lplr.Character and not vape.Modules.LongJump.Enabled then
                        ...
                        }).Magnitude * (0.9 + store.ping.total) * SpeedSlider.Value
    ...
    SpeedSlider = DamageBoost:CreateSlider({
...
run(function()
	...
	Damage = NoFall:CreateSlider({
		...
		Default = 0,
		Suffix = '%',
		Tooltip = 'Change how much damage you take'
	})
...
run(function()
    ...
    local function refreshAdornee(v)
        ...
        for _, obj in v.Frame:GetChildren() do
            if obj:IsA('Frame') and obj.Name ~= 'UIListLayout' and obj.Name ~= 'UICorner' then
                obj:Destroy()
            end
        end
...
run(function()
    ...
    local Mode
    local Whitelist
    local LowHealthWhitelist
    local LowHealthThreshold
    local UIToggle
    local Chests
    local UI
            ...
            if callback then
                ...
                repeat
                    ...
                    if entitylib.isAlive and not getShopNPC() then
                        ...
                        -- Check current health against threshold
                        local humanoid = entitylib.character and entitylib.character:FindFirstChildOfClass('Humanoid')
                        local isLowHealth = humanoid and humanoid.Health <= (LowHealthThreshold.Value or 35)
                        
                        -- Pick which list to use based on health
                        local activeWhitelist = (isLowHealth and #LowHealthWhitelist.ListEnabled > 0) and LowHealthWhitelist.ListEnabled or Whitelist.ListEnabled

                        for _, v in store.inventory.inventory.items do
                            local name = v.tool and v.tool.Name or nil
                            if name and table.find(activeWhitelist, name) and (Blacklist[name] or 0) < tick() then
                                task.spawn(function()
    ...
    LowHealthWhitelist = AutoBank:CreateTextList({
        Name = 'Low health Whitelist',
        Default = {'arrow', 'cannon', 'wool'},
        Function = function()
            if AutoBank.Enabled then
                AutoBank:Toggle()
                AutoBank:Toggle()
    ...
    LowHealthThreshold = AutoBank:CreateSlider({
        Name = 'Low health trigger',
        Min = 10,
        Max = 100,
        Default = 35,
        Function = function() end
...
run(function()
            ...
            if callback then
                repeat
                    if entitylib.isAlive and store.equippedKit == 'ice_queen' and bedwars.AbilityController:canUseAbility('ice_queen', {disableBlockedAbilityAlert = true}) then
                        local origin = entitylib.character.RootPart.Position
                        local requiredStacks = tonumber(Stacks.Value) or 3
                        for _, v in entitylib.List do
                            if v.Targetable and (v.Character:GetAttribute('IceQueenStacks') or 0) >= requiredStacks and (v.RootPart.Position - origin).Magnitude <= Range.Value then
    ...
    Stacks = AutoFreiya:CreateDropdown({
        Name = 'Stacks',
        List = {'1', '2', '3', '4'},
        Default = '3',
...
run(function()
    ...
    local function throwLasso()
        ...
        -- Check if the lasso is currently held in hand
        if store.hand.tool ~= item.tool then return end

        local source = bedwars.ItemMeta.lasso and bedwars.ItemMeta.lasso.projectileSource or nil
    ...
    AutoLasso = vape.Categories.Kits:CreateModule({
        ...
        Tooltip = 'Automatically throws Lassy\'s lasso at whoever you\'re meleeing when held'
    })
...
run(function()
    ...
    local function getLaunchTime(origin, delta, velocity, ceiling)
        ...
        for i = 1, 20 do
            for _, time in {middle * (1 + i * 0.15), middle * (1 - i * 0.045)} do
                if getLaunchVelocity(delta, Vector3.zero, time).Magnitude <= ceiling and not isPathBlocked(origin, getLaunchVelocity(delta, Vector3.zero, time), time) then
                    return time
                end
            end
        end
    ...
    DaveyAim = vape.Categories.Kits:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                local localPosition = entitylib.character.RootPart.Position
                local rawTarget = ray.Position + Vector3.new(0, entitylib.character.HipHeight, 0)
                local distance = (rawTarget - localPosition).Magnitude
                local target = rawTarget

                if distance > MaxDistance.Value then
                    local clampedDirection = (rawTarget - localPosition).Unit
                    target = localPosition + (clampedDirection * MaxDistance.Value)
                end

                local velocity = entitylib.character.RootPart.AssemblyLinearVelocity * (VelocityMultiplier and VelocityMultiplier.Value or 1)
    ...
    MaxDistance = DaveyAim:CreateSlider({
        Name = 'Max Distance',
        Min = 10,
        Max = 300,
        Default = 300,
        ...
        Tooltip = 'Clamps the maximum targeting distance to avoid going past this limit'
    })
    VelocityMultiplier = DaveyAim:CreateSlider({
        Name = 'Velocity Multiplier',
        Min = 0,
        Max = 2,
        Default = 1,
        Decimal = 10,
        Tooltip = 'Adjusts player velocity factor in trajectory calculations'
    })

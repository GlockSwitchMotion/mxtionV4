...
local hash = loadstring(downloadFile('mxtionv4/libraries/hash.lua'), 'hash')()
...
run(function()
	...
	function whitelist:announce(text)
		...
		if success then
			return sendToast({
				toastTitle = text,
				iconImage = getcustomasset('mxtionv4/assets/new/vape.png'),
				swipeUpDismiss = true,
		...
		icon.Image = getcustomasset('mxtionv4/assets/new/vape.png')
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('mxtionv4/profiles/whitelist.json') and readfile('mxtionv4/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('mxtionv4/profiles/whitelist.json', whitelist.textdata)
				end)
			end
		end
	end
...
run(function()
    ...
    local function matchesRegion(country, allowedRegions)
        if #allowedRegions == 0 then return true end
        local countryLower = string.lower(country)
        for _, region in pairs(allowedRegions) do
            if string.find(countryLower, string.lower(region)) then
                return true
            end
        end
        return false
    ...
    local function findLowestPingServerAndTeleport()
        if isScanning then return end
        isScanning = true

        notif("SetRegion", "Scanning servers for ping under 100ms...", 4, "info")
        ...
        task.spawn(function()
            ...
            for page = 1, 3 do
                if not SetRegion.Enabled then break end

                local url = 'https://games.roblox.com/v1/games/' .. game.PlaceId .. '/servers/Public?sortOrder=2&excludeFullGames=true&limit=100'
                ...
                local data = success and httpService:JSONDecode(httpdata)
                if data and data.data then
                    for _, server in pairs(data.data) do
                        if server.id ~= game.JobId and tonumber(server.playing) < playersService.MaxPlayers then
                            local serverPing = tonumber(server.ping) or 999
                            -- ONLY INCLUDE SERVERS WITH PING <= 100ms
                            if serverPing <= 100 then
                                table.insert(openServers, { id = server.id, ping = serverPing })
                            end
                        end
                    end
            ...
            if #openServers > 0 then
                local bestServer = openServers[1]
                notif("SetRegion", string.format("Found server with %dms ping! Teleporting...", bestServer.ping), 5, "info")
                task.wait(1)
                teleportService:TeleportToPlaceInstance(game.PlaceId, bestServer.id)
            else
                notif("SetRegion", "No servers under 100ms ping found right now. Retrying...", 5, "warning")
                isScanning = false
            end
        end)
    end
    ...
    local function checkRegion()
        task.spawn(function()
            task.wait(1) -- Brief pause for network stats to stabilize
            local ping = getCurrentPing()
            local country = getCurrentCountry()
            local allowed = RegionsList.List or {}

            local isPingHigh = ping > 150
            local isRegionWrong = country and not matchesRegion(country, allowed)

            if isPingHigh or isRegionWrong then
                if isPingHigh then
                    notif("SetRegion", string.format("Ping too high (%dms > 150ms). Finding new server...", ping), 5, "warning")
                else
                    notif("SetRegion", "Region mismatched (" .. (country or 'Unknown') .. "). Finding new server...", 5, "warning")
                end
                
                if AutoRequeue.Enabled then
                    findLowestPingServerAndTeleport()
                end
            else
                notif("SetRegion", string.format("Loaded successfully! Region: %s (%dms)", country or 'OK', ping), 6, "info")
            end
        end)
    end

    SetRegion = vape.Categories.Utility:CreateModule({
        Name = 'SetRegion',
        Function = function(callback)
            if callback then
                isScanning = false
        ...
        Tooltip = 'Teleports to a server under 150ms ping in your selected region'
    })

    RegionsList = SetRegion:CreateTextList({
        Name = 'Regions',
        Placeholder = 'Type region',
        Function = function() end
    })

    AutoRequeue = SetRegion:CreateToggle({
        Name = 'Auto Requeue',
        Default = true,
        Tooltip = 'Finds and teleports to servers under 150ms'
    })
end)
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('mxtionv4/assets/new/arrowmodule.png')
...
run(function()
	...
	Radar = vape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('mxtionv4/assets/new/radaricon.png'),
